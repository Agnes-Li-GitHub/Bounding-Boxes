...
bool IsOverlap(AABB p, AABB q) {
  ...
  // x and y overlap
  if (p.min[0] <= q.max[0] && q.min[0] <= p.max[0] && p.min[1] <= q.max[1] && q.min[1] <= p.max[1]) { 
    return true;
  }
  else{
    return false;
...
int main()
{
  ...
  for (int i = 0; i < numBoxes; i++) {
    ...
    cout << "AABB " << setw(2) << setfill('0') << i << ": " << boxes[i].min[0] << " " << boxes[i].max[0] << " " << boxes[i].min[1] << " " << boxes[i].max[1] << endl; 
  }
  ...
  for (int i=0;i<numBoxes;i++){
    for (int j=0;j<numBoxes;j++){
      if (IsOverlap(boxes[i], boxes[j]) && (i != j)){ // true = overlap
        flag = false;
        break;
      }
    }
